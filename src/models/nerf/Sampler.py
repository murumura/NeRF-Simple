...
class SamplePDF(torch.nn.Module):
    ...
    def forward(self, depth_values_coarse, weights, perturb:bool=True):
        ...
        interval_samples = self.sample_pdf(
            points_on_rays_mid, weights[..., 1:-1], self.uniform_x, det = (perturb == False)
        ).detach()

        depth_values_fine, _ = torch.sort(
            torch.cat((depth_values_coarse, interval_samples), dim = -1), dim = -1
        )
